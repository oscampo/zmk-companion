...
// only. Declaring one here does NOT set a value, it only makes the token
// show up (grouped under Category) in CellGridEditorForm's binding picker
// before any script has ever run `zkc --set NAME value`.
sealed class CustomTokensForm : Form
{
    ...
    {
        _tokens = existing.Select(t => new CustomTokenDef { Name = t.Name, Category = t.Category }).ToList();
        ...
        _list = new ListBox { Left = 12, Top = 62, Width = 336, Height = 150 };
        Controls.Add(_list);
        RefreshList();

        var btnRemove = new Button { Text = "Quitar", Left = 12, Top = 218, Width = 80, Height = 24 };
        btnRemove.Click += (_, _) =>
        {
        ...
        var lblName = new Label { Text = "Nombre:", Left = 12, Top = 252, Width = 60, AutoSize = false };
        Controls.Add(lblName);
        _txtName = new TextBox { Left = 76, Top = 249, Width = 100 };
        Controls.Add(_txtName);

        var lblCategory = new Label { Text = "Categoría:", Left = 182, Top = 252, Width = 62, AutoSize = false };
        Controls.Add(lblCategory);
        _txtCategory = new TextBox { Left = 246, Top = 249, Width = 102, Text = "Personalizado" };
        Controls.Add(_txtCategory);

        var btnAdd = new Button { Text = "+ Agregar", Left = 12, Top = 280, Width = 100, Height = 24 };
    ...
    private void OnAdd()
    {
        ...
        _tokens.Add(new CustomTokenDef { Name = name, Category = category });
        _txtName.Text = "";
        RefreshList();
    }
    ...
    private void RefreshList()
    {
        ...
        foreach (var t in _tokens)
            _list.Items.Add($"{{custom.{t.Name}}}  [{t.Category}]");
    }
}
